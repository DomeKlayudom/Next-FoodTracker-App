...
const ProfilePage = () => {
  ...
  const [previewImage, setPreviewImage] = useState<string | StaticImageData | null>(null);
  ...
  useEffect(() => {
    ...
    setPreviewImage(mockUser.profileImage);
  }, []);
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    console.log('Profile Data Updated:', { ...formData, profileImage: previewImage });
    alert('บันทึกข้อมูลส่วนตัวเรียบร้อยแล้ว!');
    // In a real app, you would send this data to an API to update the user profile
  };
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-r from-cyan-400 to-blue-600">
        <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-lg transform transition-all duration-300 hover:scale-105">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Password Input (optional to change) */}
            <div>
              <label htmlFor="password" className="block text-sm font-medium text-gray-700">รหัสผ่าน (กรอกหากต้องการเปลี่ยน)</label>
              <input
                type="password"
                name="password"
                id="password"
